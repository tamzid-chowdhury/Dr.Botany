...
export default class Level_Fall_Two extends GameLevel {
    ...
    currentLevel: string = Scenes.LEVEL_FALL_TWO;
	pillBottleTimer: Timer;
	trashLidTimer: Timer;
    pauseExecution: boolean = false;
    loadScene(): void {
        super.loadScene();
        this.load.tilemap("level_fall_two", "assets/tilemaps/FallLevel/level_fall_two.json"); // this change
    ...
    startScene(): void {
        ...
        this.enemyManager = new EnemyManager(this, this.viewport.getHalfSize(), [0,5,6,4,3]);
        this.emitter.fireEvent(GameEventType.PLAY_SOUND, { key: "background_music", loop: true, holdReference: true });
        
        let tilemapLayers = this.add.tilemap("level_fall_two");  // this change
        ...
        this.growthManager = new GrowthManager(this, 14);
