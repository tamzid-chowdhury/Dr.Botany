...
export class SpringLevelLayer {
	...
	constructor(scene: Scene, position: Vec2, font: string) {
		...
		this.layer = scene.addUILayer(UILayers.SPRING_LEVELS);
	...
	initButtons(): void {
		...
		{
			let sprite = this.scene.add.sprite("spring", UILayers.SPRING_LEVELS);
			sprite.position.set(startX, startY);
			sprite.scale = new Vec2(1.4, 0.8);
			sprite.alpha = 0;

			let label = <Label>this.scene.add.uiElement(UIElementType.LABEL, UILayers.SPRING_LEVELS, { position: new Vec2(startX, startY), text: `${levelSelectionArray[0]}`, size: 24 });
        ...
        {
            let sprite = this.scene.add.sprite("temp_button", UILayers.SPRING_LEVELS);
			sprite.position.set(startX, startY);
			sprite.scale = new Vec2(3, 3);
			sprite.alpha = 0;

			let label = <Label>this.scene.add.uiElement(UIElementType.LABEL, UILayers.SPRING_LEVELS, { position: new Vec2(startX, startY), text: `${levelSelectionArray[1]}`, size: 24 });
			label.size.set(430, 134);
			label.borderWidth = 0;
			label.borderRadius = 0;
			label.font = this.font;
