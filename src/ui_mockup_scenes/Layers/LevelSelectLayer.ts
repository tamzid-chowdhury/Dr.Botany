...
export class LevelSelectLayer {
	...
	initButtons(): void {
		...
		const line1 = <Label>this.scene.add.uiElement(UIElementType.LABEL, UILayers.LEVEL_SELECT, { position: new Vec2(center.x, center.y - 200), text: 'Level select layer' });
		...
		let back = <Label>this.scene.add.uiElement(UIElementType.LABEL, UILayers.LEVEL_SELECT, { position: new Vec2(startX, startY), text: "Back", size : 24});
